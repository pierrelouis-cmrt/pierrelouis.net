---
title: Flamingo Estate
order: 30
description: Photography, Motion
tags: [Graphic Design]
year: "2025"
summary: A photography and motion study focused on the ritual, texture and lived-in atmosphere surrounding Flamingo Estate.
note: Still life, interior photography and short-form motion are sequenced as one warm, tactile campaign system.
listing:
  images:
    - file: listing/01.gif
      alt: Animated Flamingo Estate campaign still life.
    - file: listing/02.webp
      alt: Warm interior photograph from the Flamingo Estate project.
    - file: listing/03.webp
      alt: Sunlit lounge from the Flamingo Estate project.
      main: true
      wide: true
---

![Sunlit lounge at Flamingo Estate](media/03.webp){wide}

The study follows small domestic rituals and lets material, light and movement carry the identity.

![Animated campaign still life](media/01.gif)
![Warm interior photograph](media/02.webp)

## Approach

Still and moving images use the same warm palette and unhurried rhythm, so the campaign can shift format without losing its atmosphere.
